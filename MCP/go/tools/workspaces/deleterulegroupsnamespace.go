...
func DeleterulegroupsnamespaceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/workspaces/%s/rulegroupsnamespaces/%s%s", cfg.BaseURL, workspaceId, name, queryString)
...
func CreateDeleterulegroupsnamespaceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_workspaces_workspaceId_rulegroupsnamespaces_name",
		mcp.WithDescription("Delete a rule groups namespace."),
		...
		mcp.WithString("workspaceId", mcp.Required(), mcp.Description("The ID of the workspace to delete rule group definition.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("The rule groups namespace name.")),
		mcp.WithString("clientToken", mcp.Description("Optional, unique, case-sensitive, user-provided identifier to ensure the idempotency of the request.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleterulegroupsnamespaceHandler(cfg),
	}
}
