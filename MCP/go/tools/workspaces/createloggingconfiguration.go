...
func CreateloggingconfigurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateLoggingConfigurationrequest
		...
		url := fmt.Sprintf("%s/workspaces/%s/logging", cfg.BaseURL, workspaceId)
...
func CreateCreateloggingconfigurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_workspaces_workspaceId_logging",
		mcp.WithDescription("Create logging configuration."),
		...
		mcp.WithString("workspaceId", mcp.Required(), mcp.Description("The ID of the workspace to vend logs to.")),
		mcp.WithString("logGroupArn", mcp.Required(), mcp.Description("Input parameter: The ARN of the CW log group to which the vended log data will be published.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CreateloggingconfigurationHandler(cfg),
	}
}
