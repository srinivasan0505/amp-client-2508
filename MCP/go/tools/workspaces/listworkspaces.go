...
func ListworkspacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/workspaces%s", cfg.BaseURL, queryString)
		...
		var result models.ListWorkspacesResponse
...
func CreateListworkspacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_workspaces",
		mcp.WithDescription("Lists all AMP workspaces, including workspaces being created or deleted."),
		...
		mcp.WithString("nextToken", mcp.Description("Pagination token to request the next page in a paginated list. This token is obtained from the output of the previous ListWorkspaces request.")),
		mcp.WithString("alias", mcp.Description("Optional filter for workspace alias. Only the workspaces with aliases that begin with this value will be returned.")),
		mcp.WithNumber("maxResults", mcp.Description("Maximum results to return in response (default=100, maximum=1000).")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListworkspacesHandler(cfg),
	}
}
