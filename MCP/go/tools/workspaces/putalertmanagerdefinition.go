...
func PutalertmanagerdefinitionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePutalertmanagerdefinitionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_workspaces_workspaceId_alertmanager_definition",
		mcp.WithDescription("Update an alert manager definition."),
		...
		mcp.WithString("workspaceId", mcp.Required(), mcp.Description("The ID of the workspace in which to update the alert manager definition.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PutalertmanagerdefinitionHandler(cfg),
	}
}
